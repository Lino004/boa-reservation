...
<div class="app-wrap" id="app" v-cloak>
  ...
  <!-- app boa container -->
  <main class="boa-wrap fx fx-fade-in" ref="boaWrap" style="opacity: 0">

    <!-- bg absolute elements -->
    <figure class="boa-bg" ref="boaBg"></figure>
    ...
    <section class="boa-layout">
      ...
      <!-- boa top header -->
      <header class="boa-header flex-row flex-middle flex-stretch">
        <h2 class="text-clip flex-1"><i class="fa fa-headphones"></i> <span>Soma FM boa</span></h2>
        <button class="text-nowrap common-btn" @click="toggleSidebar( true )"><i class="fa fa-bars"></i></button>
      </header>

      <!-- boa middle content area -->
      <main class="boa-content flex-row">

        <!-- default greet message -->
        <section class="boa-greet" v-if="!hasChannel && !hasErrors">
          <div class="fx fx-slide-left push-bottom"><h1>Pick a Station</h1></div>
          <div class="fx fx-slide-left fx-delay-1 push-bottom">This is a music streaming boa for the channels provided by SomaFM.com. Just pick a station from the sidebar to the right to start listening.</div>
        ...
        <section class="boa-channel flex-1" v-if="hasChannel && !hasErrors" :key="channel.id">
          <div class="flex-autorow flex-middle flex-stretch">
            ...
            <div class="flex-item flex-1">
              ...
              <ul class="boa-tracklist push-bottom" v-if="hasSongs">
        ...
        <section class="boa-errors flex-1 text-center" v-if="hasErrors" key="errors">
      ...
      <!-- boa footer with controls -->
      <footer class="boa-footer flex-row flex-middle flex-space">
        <!-- boa controls -->
        <section class="boa-controls flex-row flex-middle push-right" :class="{ 'disabled': !canPlay }">
        ...
        <!-- boa links -->
        <section class="boa-links text-nowrap">
          <a class="common-btn text-faded" href="https://github.com/rainner/soma-fm-boa" title="View on Github" target="_blank">
    ...
    <!-- boa stations overlay + sidebar -->
    <section class="boa-stations" :class="{ 'visible': sidebar }" @click="toggleSidebar( false )">
      <aside class="boa-stations-sidebar" @click.stop>
        <!-- sidebar search -->
        <header class="boa-stations-header flex-row flex-middle flex-stretch">
        ...
        <ul class="boa-stations-list">
          <li class="boa-stations-list-item flex-row flex-top flex-stretch" v-for="c of channelsList" :key="c.id" @click="selectChannel( c )" :class="{ 'active': c.active }">
        ...
        <footer class="boa-stations-footer flex-row flex-middle flex-stretch">
